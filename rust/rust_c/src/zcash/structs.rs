...
///
/// Opaque to C: this is a plain Rust enum (deliberately not `#[repr(C)]`), and C
...
pub enum BatchDisplayCache {
    /// The split transaction (message 0, full per-output review) plus ONE
    /// aggregate migration summary folding in the remaining messages.
    SplitPlusSummary {
        split: ParsedPczt,
        summary: ParsedPczt,
    },
    /// One display row per message: the ordinary per-message review.
    PerMessage(Vec<ParsedPczt>),
}
