...
pub(crate) struct CheckedShieldedParse {
    ...
    pub(crate) swept_actions: alloc::vec::Vec<SweptAction>,
}

#[cfg(feature = "cypherpunk")]
pub(crate) struct SweptAction {
    ...
    pub(crate) output_value: Option<u64>,
...
pub(crate) fn check_and_parse_pczt_shielded<P: consensus::Parameters>(
    ...
    let mut swept_actions = alloc::vec::Vec::new();
...
fn check_and_parse_shielded_bundle<P: consensus::Parameters>(
    ...
    bundle: &orchard::pczt::Bundle,
    pool: ShieldedPool,
    swept_actions: &mut alloc::vec::Vec<SweptAction>,
    ...
    bundle
        ...
        .try_for_each(|(index, action)| {
            ...
            let output_value = action.output().value().map(|value| value.inner());
            let output_recipient_owned = match output_value {
                Some(value) if value != 0 => action.output().recipient().map(|recipient| {
                    super::parse::is_wallet_orchard_address(keys, &recipient).unwrap_or(false)
                }),
                _ => None,
            };
            swept_actions.push(SweptAction {
                pool,
                index,
