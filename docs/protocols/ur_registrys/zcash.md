...
and signing, and shows the selected network in the transaction review UI. The
batch envelope `network` field remains `zcash-mainnet` in version 1 because the
registry currently defines only that value. It is not used to enable testnet.
When the registry defines a testnet value, firmware should reject envelopes that
do not match the selected device network.
